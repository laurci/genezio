...
class {{{className}}} {
  private val remote = Remote(URI.create("{{{_url}}}"))

  {{#methods}}
    fun {{{name}}}({{#parameters}}{{{name}}}{{^last}}, {{/last}}{{/parameters}}): {{{returnType}}} {
        val params = listOf<JsonElement>(
          {{#sendParameters}}
          Json.encodeToJsonElement({{{name}}}){{^last}}, {{/last}}
          {{/sendParameters}}           
        )

        {{#returnTypeCast}}
        val res = remote.makeRequest({{{methodCaller}}}, params)
        return Json.decodeFromJsonElement<{{{returnType}}}>(res)
        {{/returnTypeCast}}
        {{^returnTypeCast}}
        val res = remote.makeRequest({{{methodCaller}}}, params)
        {{/returnTypeCast}}
