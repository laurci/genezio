...
export class AstGenerator implements AstGeneratorInterface {
    ...
    async #compileGenezioKotlinAstExtractor() {
        ...
        await runNewProcess("git clone https://github.com/Genez-io/kotlin-ast.git .", folder)
        await runNewProcess("./gradlew run", folder)
        await runNewProcess("./gradlew distZip", folder)
        if (!fs.existsSync(path.join(os.homedir(), ".kotlin_ast_generator"))) {
            fs.mkdirSync(path.join(os.homedir(), ".kotlin_ast_generator"));
        }
        await runNewProcess(`unzip app/build/distributions/app.zip`, folder)
        await runNewProcess(`mkdir ${os.homedir()}/.kotlin_ast_generator/bin ${os.homedir()}/.kotlin_ast_generator/lib`, folder)
        await runNewProcess(`mv ./app/bin/ ./app/lib/ ${os.homedir()}/.kotlin_ast_generator/`, folder)
    }
    ...
    async generateAst(input: AstGeneratorInput): Promise<AstGeneratorOutput> {
        // Check if kotlin and gradle are installed
        checkIfKotlinReqsAreInstalled();

        // Check if the kotlin ast extractor is compiled and installed in home.
        const genezioAstGeneratorPath = path.join(os.homedir(), ".kotlin_ast_generator", "bin", "app");
        ...
        const result = await runNewProcessWithResult(`${os.homedir()}/.kotlin_ast_generator/bin/app ${classAbsolutePath}`)
